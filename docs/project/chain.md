...
On this server the genesis account's handle is Tim. Other servers may run their
own genesis account under another name, or share this one; the docs say
*genesis account* because the handle is only a handle.
...
A new account starts with the genesis account as its first friend.
