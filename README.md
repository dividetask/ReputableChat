...
**Test traffic** is bots, one process per account, signing the same records a
browser signs. Each declares a category — `scammer`, `troll`, `gullible`,
`expert` and three more — which briefs the model behind it and decides who it
gravitates towards. They start invisible and have to be vouched for like
anybody else, some of them abandon their account every few days and come back
as somebody new, and no link one posts goes anywhere but a page explaining why
you should not have clicked it.
