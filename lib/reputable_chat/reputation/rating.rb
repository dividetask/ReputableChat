...
module ReputableChat
  module Reputation
    ...
    Rating = Struct.new(:friend, :reported, :net_votes, keyword_init: true) do
      ZERO     = BigDecimal("0")
      ONE      = BigDecimal("1")
      ...
      def to_h
        { "friend" => !!friend, "reported" => !!reported, "net_votes" => net_votes.to_i }
      end
      ...
      def value(curve:, friend_value:)
        ...
        raw = curve.value(net_votes.to_i)
        raw += friend_value if friend
        raw.clamp(NEGATIVE, ONE)
      end
    end
  end
end
